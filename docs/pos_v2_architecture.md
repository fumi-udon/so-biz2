...
*   **永続化:** `localStorage` (VueUse の `useLocalStorage` 等を活用し、マスタデータとカート情報を保持)
*   **禁止事項:**
...
*   **ドラフト（未送信カート）のキー:** 必ずセッション単位で独立させること。
...
*   **POS V2 (Vue/Inertia) が担当する領域:**
    ...
    *   商品一覧・カテゴリタブの表示
    *   商品追加モーダル（トッピング・スタイル選択）とカート（Draft）の管理
    *   バックエンドへの注文データの一括送信（POST）
...
*   **完全なUIロック:** 送信処理中（`isOrderSubmitting` === `true`）は、カート内だけでなく、商品グリッド（CategoryRail / ProductGrid）を含む**画面全体の操作を半透明オーバーレイ等で物理的にロック**すること。
...
*   **ペイロードと冪等性の担保:** API へは `{ shop_id, table_session_id, lines, client_submit_id }` を送信する。`client_submit_id` には送信のたびに一意の UUID を付与し、将来的なサーバー側での重複排除の布石とする。
*   **完了時の状態リセット:** **HTTP 201（成功）** を受信した場合のみ、Draft の `lines` を空にし、LocalStorage の該当ドラフトキーを破棄すること。2xx でも 201 以外は成功扱いにしない（契約の揺れ防止）。
...
*   **フロント:** `resources/js/pos2/utils/orderSubmitInvestigation.js` — `pushOrderSubmitTrace` / `recordLastOrderSubmitAudit`。`useDraftStore.submitOrder` からのみ呼ぶ。内部は `try-catch` と `page.props.auth.debug`（`POS2_DEBUG`）ガードで、本処理を止めない。
...
*   **DebugPanel:** Cart タブに `lastOrderSubmit`（`outcome`, `http_status`, `duration_ms`, `trace_id` 等）と `isOrderSubmitting` を表示し、Trace タブの時系列と対応付け可能にする。
...
*   **本番死守 (Non-intrusive Try-Catch):**
    調査コードの内部でパースエラーや参照エラーが起きても、呼び出し元の処理（テーブル選択、カート追加、注文送信など）を決して中断させないこと。すべての調査コードは `try-catch` で保護し、エラー時は安全に握りつぶすこと。
...
*   **右ペイン表示:** ゲスト経由の注文と `POST .../orders`（Add to Table）は **同一 `orders[]` にマージ**され、行ごとに `is_unsent` で KDS 送信前後を判別する（Vue: [`SessionRightColumn.vue`](resources/js/pos2/components/SessionRightColumn.vue)）。
...
*   **失敗（業務バリデーション）:** HTTP **422**。JSON の `message` を人間可読で返す。フロント（`CartPanel`）は **同文言をアラート表示**し、ドラフトは **201 以外では消さない**（部分成功は行わず、DB は外側トランザクションでロールバック）。
*   **楽観 UI（0ms）:** 送信ボタン押下の**同一ティック**で `sessionOrdersPayload.orders` に仮注文（`id: opt:{client_submit_id}`・`is_unsent: true`）をマージしカートを空にする。`POST` / 成功後の `GET .../orders` / `GET table-dashboard` は裏で実行し、`applySessionOrdersJson` で仮行を権威データに置換。POST 失敗時のみ仮行除去＋ドラフト復元＋アラート。詳細は **§10-11**。
...
*   **ドラフト（`adding` 時）:** 右カラム内を `lg:flex-row` で **メニュー（左）+ `SessionRightColumn`（右）**。`CartPanel` の **Add to Table** は `POST .../orders`（KDS は飛ばさず `Placed` のみ）。

### 10-8. セッション無しの卓

*   アクティブ `table_sessions` が無い卓でも **Add to Table** は `POST /pos2/api/tables/{restaurant_table_id}/orders` で [`TableSessionLifecycleService::getOrCreateActiveSession`](app/Services/Pos/TableSessionLifecycleService.php) によりセッション確定後に `Placed` 登録する。フロントは `pendingTableId` でドラフトを保持し、201 の `table_session_id` を受けたら **`patchActiveSessionId` のみ**で同期する（`syncSelectionFromTile` は `sessionOrdersPayload` を null にするため送信完了フローでは使わない）。

### 10-9. Add to Table トラブルシュート（調査メモ）

*   **`Style selection is required.`**  
    *   同一カートに **スタイル必須商品の「スタイルなし行」**（旧フローでグリッド即時追加など）が残ると、POST 内の **先頭失敗行で 422** となり **全体ロールバック**する。  
    *   対策: `rules.style_required === true` の商品は **`ProductGrid` / `Index.onAddSimple` で必ず ConfigModal** へ。送信前は `useDraftStore.submitOrder` がマスタ照合でブロック（日本語メッセージ）。  
...
*   **本番の定期整理:** 未送信ドラフトを失わないよう、**Recu 成功後・会計クローズ後**などドメインイベントにフックして「当該セッションのドラフトキーだけ `removeItem`」する方が安全。全消しはオペ用・ステージング向け。

### 10-11. Add to Table 楽観マージ（B+C）

*   **`usePos2SessionUiStore.appendOptimisticStaffSubmit`:** `axios.post` の**前**に、送信中の `lines[]` を `orders[]` に仮 1 注文として足す（各行 `is_unsent: true`, `ordered_by: staff`）。直後 **`clearDraft`** でカートを空にし、右ペインは薄赤（KDS前）で即表示。
...
*   **ロールバック:** POST 非 201 / ネットワーク失敗時のみ `removeOptimisticStaffSubmit`＋ドラフト行復元＋アラート。POST 成功後の GET 失敗ではカートに戻さない（DB 済みのため）。楽観再検証中は読み込みスピナーを出さない（`skipLoadingUi`）。